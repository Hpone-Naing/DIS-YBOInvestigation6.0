...
namespace YBOInvestigation.Services.Impl
{
    public class AbstractServiceImpl<T> : AbstractService<T> where T : class
    {
        ...
        public AbstractServiceImpl(YBOInvestigationDBContext context, ILogger<AbstractServiceImpl<T>> logger)
        {
            _context = context;
            _logger = logger;
        }
                    ...
                    foreach (PropertyInfo prop in obj.GetType().GetProperties())
                    {
                        if (prop.PropertyType == typeof(string))
                        {
                            if (column == null || string.Equals(prop.Name, column, StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                if (propValue != null)
                                {
                                    if (propValue.Trim().IndexOf(searchData.Trim(), StringComparison.OrdinalIgnoreCase) >= 0)
                                    {
